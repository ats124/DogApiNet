...
namespace DogApiNet
{
    public interface ITagApi
    {
        ...
        Task<ILookup<string, string>> GetBySourceAsync(string hostName, string source = null, CancellationToken? cancelToken = null);
    }
    ...
    partial class DogApiClient : ITagApi
    {
        ...
        async Task<string[]> ITagApi.GetAsync(string hostName, string source, CancellationToken? cancelToken)
        {
            ...
            if (source != null) @params["source"] = source;
            var result =
                await RequestAsync<DogTagGetResult>(HttpMethod.Get, $"/api/v1/tags/hosts/{Uri.EscapeDataString(hostName)}", @params, null,
                        cancelToken)
                    .ConfigureAwait(false);
            return result?.Tags ?? new string[0];
        }

        async Task<ILookup<string, string>> ITagApi.GetBySourceAsync(string hostName, string source, CancellationToken? cancelToken)
        {
            var @params = new NameValueCollection();
            if (source != null) @params["source"] = source;
            @params["by_source"] = "true";
            var result =
                await RequestAsync<DogTagGetBySourceResult>(HttpMethod.Get, $"/api/v1/tags/hosts/{Uri.EscapeDataString(hostName)}", @params, null,
                        cancelToken)
                    .ConfigureAwait(false);
            if (result?.Tags == null)
                return new string[0].ToLookup(x => x);
