        ...
        static async Task Main(string[] args)
        {
            ...
            using (var client = new DogApiClient(apiKey, appKey))
            {
                ...
                var monitor = await client.Monitor.CreateAsync(
                    new DogMonitor(DogMonitorTypes.MericAlert, "max(last_1m):avg:test.random{*} > 90")
                    {
                ...
                getMonitor.Name = "test";
                var updateMonitor = await client.Monitor.UpdateAsync(getMonitor);
                Debug.Assert(updateMonitor.Name == "test");

                var tags2 = new[] { "testtag3" };
                var silenced2 = DateTimeOffset.Now.AddHours(6);
                var editMonitor = await client.Monitor.UpdateAsync(
                    new DogMonitor(monitor.Id, "max(last_1m):avg:test.random{*} > 80")
                    {
