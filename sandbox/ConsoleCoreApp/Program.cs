    ...
    static class Program
    {
        static async Task Main(string[] args)
        {
            ...
            using (var client = new DogApiClient(apiKey, appKey))
            {
                ...
                var getResult = await client.Timeboard.GetAsync(allResult.Timeboards[0].Id);
                var createResult = await client.Timeboard.CreateAsync(getResult.Timeboard);
                var updateResult = await client.Timeboard.UpdateAsync(createResult.Timeboard);
                await client.Timeboard.DeleteAsync(updateResult.Timeboard.Id);
            }
        }
